...
interface IVESTArOrganizerRegistry is IVESTArAdminControl {
    ...
    event OrganizerProfileUpserted(address indexed organizer, bytes32 indexed displayNameHash, string brandMetadataURI);

    // verified 상태가 바뀌는 순간을 기록하는 이벤트
    event OrganizerVerificationUpdated(
        address indexed organizer, bool verified, uint64 effectiveTime, uint64 revokedTime
    );
    ...
    function setVerification(address organizer, bool verified, uint64 effectiveTime, uint64 revokedTime) external;

    // view returns (...) : 상태 변경 없이 값을 읽어오고, OrganizerProfile struct를 통째로 반환
    function getOrganizerProfile(address organizer) external view returns (VESTArTypes.OrganizerProfile memory);
